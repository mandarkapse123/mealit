...
// Dynamically discover supported models and call Gemini
async function callGeminiDynamic(apiKey, contents, systemPrompt = '') {
  ...
  try {
    ...
    if (listData.models && listData.models.length > 0) {
      // Find flash or pro model that supports generateContent
      const valid = listData.models.find(m => 
        m.supportedGenerationMethods?.includes('generateContent') &&
        (m.name.includes('flash') || m.name.includes('pro') || m.name.includes('gemini'))
      );
      if (valid) {
        targetModelName = valid.name; // e.g. "models/gemini-1.5-flash-001"
      }
    } else if (listData.error) {
      throw new Error(`Google AI Key Error: ${listData.error.message}`);
    }
  } catch (err) {
  ...
  const modelPath = targetModelName.startsWith('models/') ? targetModelName : `models/${targetModelName}`;
      ...
      const contents = [
        {
          ...
          parts: [
            ...
            {
              text: 'Please listen to this audio message carefully and answer the question according to the family meal database.'
            }
          ]
        }
      ];
